...
When `obj.setAgeThis(27)` is called -- showing how `this` works:

```
Stack                                        Heap
┌───────────────────────┐                   ┌──────────────────────────┐
│ setAgeThis(int age)   │                   │ Human object (obj)       │
│  ┌─────────────────┐  │                   │ ┌──────────────────────┐ │
│  │ age = 27 (local)│  │                   │ │ private age  = 27    │ │
│  │                 │  │                   │ │ private name ="Ashish│ │
│  │ this ───────────┼──┼──────────────────►│ └──────────────────────┘ │
│  └─────────────────┘  │                   └──────────────────────────┘
│                       │                              ▲
│  this.age = age;      │   "this.age" refers to       │
│  (instance)  (local)  │    the object's field ────────┘
├───────────────────────┤
│ main()                │
│  ┌─────────────────┐  │
│  │ obj ────────────┼──┼──────────────────────────────┐
...
"this" and "obj" both point to the SAME object on the Heap!
```
...
┌───────────────────┐               └──────────────────────────┘
│ obj1             │───────────────►┌──────────────────────────┐
│ (reference)      │               │ Human object             │
└───────────────────┘               │  age  = 20               │
                                    │  name = "John2"          │
                                    └──────────────────────────┘

Note: "this" is an implicit reference available inside every instance method.
      It always points to the object that called the method.
      When obj.setAgeThis(27) runs, "this" == obj (same reference).
      When obj1.setAgeThis(x) would run, "this" == obj1.
```