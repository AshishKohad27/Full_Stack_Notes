...
-- 2. Create DB
-- CREATE DATABASE IF NOT EXISTS television_shows;

-- 3. Use DB: select DB so that we can do CRUD further.
-- use television_shows;

-- 3.1 Drop DB
-- DROP DATABASE television_shows;
...
-- Point 1 :- title and rating
-- SELECT title, rating FROM series JOIN reviews ON series.id = reviews.series_id;

-- Point 2 : Group title and avg rating
-- SELECT title, ROUND(AVG(rating), 2) AS 'avg_rating' FROM rating JOIN reviews ON series.id = reviews.series_id GROUP BY title ORDER BY avg_rating;

-- Point 3 : 
-- SELECT first_name, last_name, rating FROM reviewers JOIN reviews ON reviewers.id = reviews.reviewer_id;

-- Point 4 : find unreviewed_series
